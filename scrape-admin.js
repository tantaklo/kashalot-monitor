...
async function fetchOrdersForDate(jar, csrf, date) {
  const params = new URLSearchParams({
    ...
    'columns[0][data]': 'id',
    'columns[1][data]': 'client_id',
    'columns[2][data]': 'object_id',
    'columns[3][data]': 'tariff_id',
    'columns[4][data]': 'subscription_id',
    'columns[5][data]': 'status',
    'columns[6][data]': 'payment',
    'columns[7][data]': 'amount',
    'columns[8][data]': 'card_amount',
  ...
  if (res.status !== 200) {
    console.log('DEBUG orders/search body[:500]:', res.body.replace(/\s+/g, ' ').slice(0, 500));
